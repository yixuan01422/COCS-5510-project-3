...
class InsertHandler:
    ...
    def handle(self, parsed):
        ...
        values = [] # Stores values to be inserted

        # Extract table name and values to insert
            ...
            if token.ttype is sqlparse.tokens.Keyword and token.value.upper() == 'INTO':
                ...
                for value_token in value_tokens:
                    ...
                    if value_token.isdigit():
                        values.append(int(value_token)) # Convert to integer if applicable
                    elif value_token.startswith("'") and value_token.endswith("'"):
                        values.append(value_token.strip("'")) # Strip single quotes for strings
                    else:
